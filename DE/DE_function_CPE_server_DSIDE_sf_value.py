...
class DSIDE_Algorithm():
    ...
    def optimize(self,filename:str,cr_reversed:bool=False,sf_reversed:bool=False,cr:float = 0.8,sf:float = 0.2):
        ...
        scaling_factor_min = np.zeros(self.round)
        scaling_factor_max = np.zeros(self.round)
        scaling_factor_mean = np.zeros(self.round)

        scaling_factor_value_data = np.zeros((self.round,3))
        
        scaling_factor_inpopulation = np.zeros(self.populationsize)
        crossover_rate = cr
        scaling_factor = sf
        for k in range(replace):
            ...
            for i in range(self.round):
                ...
                for j in range(self.populationsize):
                    if not self.control:
                        ...
                        if not self.sf_fix:
                            ...
                            scaling_factor_inpopulation[j] = scaling_factor
                    else:
                        if not self.cr_fix:
                            if not cr_reversed:
                ...
                scaling_factor_value_data[i][0] = np.max(scaling_factor_inpopulation)
                scaling_factor_value_data[i][1] = np.min(scaling_factor_inpopulation)
                scaling_factor_value_data[i][2] = np.mean(scaling_factor_inpopulation)
                for j in range(self.populationsize):
            ...
            file_path = self.main_path+'/scaling_result/'+self.functionname+'_result/'+self.functionname+'_'+self.filename+'_run_'+str(k+1)+'.csv'
            with open(file_path,'w',encoding='UTF8',newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['Max','Min','Mean'])
                writer.writerows(scaling_factor_value_data)
        ...
        scaling_factor_analysis_max = np.zeros((replace,self.round))
        scaling_factor_analysis_min = np.zeros((replace,self.round))
        scaling_factor_analysis_mean = np.zeros((replace,self.round))

        scaling_max_average = np.zeros((1,self.round))
        scaling_min_average = np.zeros((1,self.round))
        scaling_mean_average = np.zeros((1,self.round))

        scaling_average_value = np.zeros((self.round,3))
        for k in range(replace):
            path = self.main_path+'/scaling_result/'+self.functionname+'_result/'+self.functionname+'_'+self.filename+'_run_'+str(k+1)+'.csv'
            my_sol = pd.read_csv(path,nrows=self.round)
            #print(my_sol)
            scaling_factor_analysis_max[k:] = my_sol['Max']
            scaling_factor_analysis_min[k:] = my_sol['Min']
            scaling_factor_analysis_mean[k:] = my_sol['Mean']
        
        for j in range(self.round):
            for i in range(replace):
                scaling_average_value[j][0] = scaling_average_value[j][0]+(scaling_factor_analysis_max[i][j]/replace)
                scaling_average_value[j][1] = scaling_average_value[j][1]+(scaling_factor_analysis_min[i][j]/replace)
                scaling_average_value[j][2] = scaling_average_value[j][2]+(scaling_factor_analysis_mean[i][j]/replace)
        
        file_path = self.main_path+'/Scaling_Graph/'+self.functiontype+'/value/'+self.functionname+'_'+self.filename+'_scalingvalue'+'.csv'
        with open(file_path,'w',encoding='UTF8',newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Max','Min','Mean'])
            writer.writerows(scaling_average_value)
    
        scaling_show = np.transpose(scaling_average_value)
        ...
        print('Scaling range: Max ',np.mean(scaling_show[0]),' Min ',np.mean(scaling_show[1]),' Mean ',np.mean(scaling_show[2]))
        print('=================================================================')
