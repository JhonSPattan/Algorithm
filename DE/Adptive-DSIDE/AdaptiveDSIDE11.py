...
class DSIDEselectBestMix():
    ...
    def sleectionBestMutant(self,component:np.array)->np.array:
        ...
        newVector1 = np.zeros(self.dimension)
        newVector2 = np.zeros(self.dimension)
        for i in range(self.dimension):
            newVector1[i] = np.median(dummyList[:,i])

        for i in range(self.dimension):
            newVector2[i] = np.mean(dummyList[:,i])

        mutantList.append(newVector1)
        mutantList.append(newVector2)
